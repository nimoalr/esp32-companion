...
static void render_task(void *arg)
{
    ...
    for (;;) {
        ...
        if (now_us - stats_t0 >= STATS_PERIOD_US && frames) {
            ...
            char audio_s[96] = "";
            if (af.active) {
                snprintf(audio_s, sizeof audio_s, " | audio %" PRIu32 " us, rms %.1f peak %d LSB, loud %.2f bass %.2f, beats %" PRIu32 " %d bpm",
                         af.cpu_us, af.raw_loud, (int)af.peak, af.loud, af.bass, af.beat_count, (int)af.bpm);
            }
