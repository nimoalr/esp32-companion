 ...
 *
 * A shape is a rounded rectangle with an independent radius per corner
 * (a capsule when all four equal min(hw, hh)), optionally rotated, cut by
...
typedef struct {
    ...
    int32_t rad[4];         /* corner radii TL, TR, BL, BR (local frame), each <= min(hw, hh) */
    ...
    bool rot;
    int32_t rc, rs;
    bool rad_equal;         /* all four radii equal (set by raster_shape_finalize) */
    /* Float mirror of the geometry for the rotated / bent-lid per-row paths (set by raster_shape_finalize) */
    float fc, fs, frad[4], fhw, fhh, ftop, fbot, fslant, fcurve, fbend, finv_hw2;
