...
| Efficient music calibration with local HTML coordination | Music Lab records labeled feature references over USB, about 240 KB/minute in dedicated mode with the renderer paused, with raw 16-band spectra, sequence/CRC checks, markers/export/import and a plain-C report tool. It does not automatically train or tune firmware thresholds. Further tuning is pending the planned recordings. |
