...
int main(void)
{
    FILE *video=fopen("tools/host/out/dance-organic.rgb","wb");
    if(!video){perror("dance-organic.rgb");return 1;}
    const char *labels[]={"ORGANIC DISCO","BURST / RECOIL","PINBALL HOPS","BOTH / DISCO"};
    for(int i=0;i<4;i++) {
        ...
        s->sm.dance_visual=(i==0||i==3)?2:0;s->sm.dance_visual_ms=0;s->sm.dance_visual_len=100000;
        s->sm.dance_lasers_on=i==3;s->sm.dance_laser_len=100000;
        s->sm.dance_spots_on=i==3;s->sm.dance_spot_len=100000;
    }
    ...
    for(uint32_t frame=0;frame<SECONDS*60;frame++) {
        ...
        for(int i=0;i<4;i++) {
            scene_t *s=&scenes[i];if((i==1||i==2)&&frame==240){s->sm.rush_ms=t;s->sm.rush_kind=i;}anim_set_audio(&s->sm,&a);anim_update(&s->sm,&s->eyes,t);
            eyes_update(&s->eyes,t,s->sh);
            dance_background_update(&s->lasers,s->eyes.laser_mix,s->eyes.spot_mix,&a,t+i*43,s->eyes.face_deg);
            ...
            for(int y=0;y<W+64;y+=32) {
                ...
                const char *state=(i==1||i==2)&&t>=4000&&t<6000?"SHORT LOOP / 2s BURST":"150 BPM";
