/* Replays 16 kHz mono/stereo PCM WAV through the exact firmware analyser. */
#include <stdio.h>
#include <time.h>
...
int main(int argc,char **argv)
{
 ...
 unsigned bytes=0;int valid=0,channels=0;
 while(fread(h,1,8,f)==8){unsigned n=u32(h+4);if(!memcmp(h,"fmt ",4)){
  if(n<16||fread(h+8,1,16,f)!=16)return 2;
  channels=h[10];valid=h[8]==1&&h[9]==0&&(channels==1||channels==2)&&h[11]==0&&u32(h+12)==16000&&h[22]==16&&h[23]==0;fseek(f,(long)n-16+(n&1),SEEK_CUR);
 }else if(!memcmp(h,"data",4)){bytes=n;break;}else fseek(f,n+(n&1),SEEK_CUR);}
 if(!valid||bytes<FRAME*2*channels||bytes%(2*channels))return 2;
 int16_t *pcm=malloc(bytes);if(!pcm||fread(pcm,1,bytes,f)!=bytes)return 2;fclose(f);
 unsigned values=bytes/2,samples=values/channels;double sum=0;for(unsigned i=0;i<values;i++)sum+=(double)pcm[i]*pcm[i];
 float target=argc>2?atof(argv[2]):300;
 float gain=target>0 && sum>0 ? target/sqrt(sum/values) : 1.f;
 FILE *csv=argc>3?fopen(argv[3],"w"):NULL;
 if(argc>3&&!csv){perror(argv[3]);free(pcm);return 1;}
 if(csv)fprintf(csv,"time_ms,beat,kick,raw_loud,bpm,confidence,ratio,speech,kick_raw,bass_raw,mid_raw,high_raw,would_dance,would_listen,music_conf,music_bpm,music_evidence,dance_drive\n");
 ...
 for(unsigned pos=0;pos+FRAME<=samples;pos+=FRAME){
  int16_t stereo[FRAME*2];for(int i=0;i<FRAME;i++)for(int ch=0;ch<2;ch++){float v=pcm[(pos+i)*channels+(channels==2?ch:0)]*gain;if(v>32767)v=32767;if(v< -32768)v=-32768;stereo[2*i+ch]=(int16_t)v;}
  ...
  if(csv)fprintf(csv,"%u,%d,%.5f,%.2f,%.2f,%.3f,%.3f,%d,%.6f,%.6f,%.6f,%.6f,%d,%d,%.3f,%.2f,%.3f,%.3f\n",pos/16,beat,s_feat.kick,s_feat.raw_loud,s_feat.bpm,s_feat.tempo_conf,s_feat.bass_ratio,s_feat.speech,s_feat.kick*s_max_kick/fmaxf(.001f,s_presence),s_feat.bass*s_max_bass/fmaxf(.001f,s_presence),s_feat.mid*s_max_mid/fmaxf(.001f,s_presence),s_feat.high*s_max_high/fmaxf(.001f,s_presence),beh.state==BEH_MUSIC,beh.state==BEH_LISTENING,s_feat.music_conf,s_feat.music_bpm,s_feat.music_evidence,s_feat.dance_drive);
 }
 if(csv)fclose(csv);
