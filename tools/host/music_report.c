...
int main(int argc,char **argv){
 ...
 unsigned char p[24];if(fread(p,1,12,f)!=12||memcmp(p,"MCALv001",8)){fprintf(stderr,"invalid header\n");return 2;}
 ...
 fprintf(csv,"frame,device_ms,session,raw_rms,kick_rms,mean_rms,previous_rms,bpm,presence,bass_ratio,confidence,speech_depth,candidate,beat,speech,own_voice,clipped,rush,dancing,listening,bass,mid,high\n");
 unsigned frames=0,beats=0,rush=0;size_t got;
 while((got=fread(p,1,24,f))==24){unsigned flags=p[18];beats+=!!(flags&2);rush+=!!(flags&32);
 ...
 for(int b=0;b<8;b++)fprintf(csv,",%u",!!(flags&(1u<<b)));fprintf(csv,",%.4f,%.4f,%.4f\n",p[19]/255.,p[20]/255.,p[21]/255.);
 }
 ...
 printf("%u frames, %u bytes of features, %u accepted beats, %u rush events; run boundaries/markers in %s\n",frames,frames*24,beats,rush,argv[3]);return 0;
}
