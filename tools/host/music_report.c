...
int main(int argc,char **argv){
 ...
 unsigned char p[1092];if(fread(p,1,12,f)!=12||(memcmp(p,"MCALv001",8)&&memcmp(p,"MCALv002",8)&&memcmp(p,"MCALv003",8)&&memcmp(p,"MCALv004",8))){fprintf(stderr,"invalid header\n");return 2;}
 int portable=p[7]=='4';unsigned bytes=p[7]=='3'?1092:p[7]=='2'?64:24;
 unsigned n=u32(p+8),record_count=0;if(n>16000000){fprintf(stderr,"metadata too large\n");return 2;}
 if(portable){
  if(fread(p,1,12,f)!=12){fprintf(stderr,"truncated portable header\n");return 2;}
  bytes=u32(p);record_count=u32(p+4);unsigned attachments=u32(p+8);
  if(bytes!=24&&bytes!=64&&bytes!=1092){fprintf(stderr,"invalid record size\n");return 2;}
  uint64_t expected=24ull+n+(uint64_t)record_count*bytes+attachments;
  if(expected>1000000000ull||fseek(f,0,SEEK_END)||ftell(f)!=(long)expected||fseek(f,24,SEEK_SET)){fprintf(stderr,"invalid portable session length\n");return 2;}
 }
 ...
 unsigned frames=0,beats=0,rush=0;size_t got=0;
 while((!portable||frames<record_count)&&(got=fread(p,1,bytes,f))==bytes){unsigned flags=p[18];beats+=!!(flags&2);rush+=!!(flags&32);
 ...
 error|=(portable?frames!=record_count:got!=0)||ferror(f)||ferror(csv);error|=fclose(f)!=0;error|=fclose(csv)!=0;
